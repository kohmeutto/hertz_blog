...
$$
\vec{h}_i=\frac{\partial\vec{v}'}{\partial u_i'} \text{, 매개변수의 미소변화량에 대한 벡터 v의 미소변화량 - 궤적의 접선 벡터}
$$
...
- **매개변수 공간** 에서, 매핑 위치의 미소변위벡터

$$
d\vec{l}_{C}'=\left[d\rho',d\phi',dz'\right]^T
$$

- **실 공간** 에서, 매핑 위치의 미소변위벡터
...
$$
d\vec{l}_{C}'\xrightarrow{\text{mapping: }\cdot\nabla_{C}}d\vec{l}_{D}'
$$
...
$$
d\vec{l}_{S}'\xrightarrow{\text{mapping: }\cdot\nabla_{S}}d\vec{l}_{D}'
$$
