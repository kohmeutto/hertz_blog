...
스펙트럼 함수 $A_m(E,\mathbf{k},V)$의 반치폭(FWHM)과의 연결을 하면, $A_m(E)$의 형태는 다음과 같다.
...
$$
J(V)=\frac{e}{h}\Delta F(V)\int_{E_{min}}^{\infty}dE\exp\left(-\frac{E}{k_BT}\right)\left[\frac{1}{2}+\frac{1}{\pi}\arctan\left(\frac{E-\Phi_B(V)}{\eta_0}\right)\right]
$$

**6) 부분 적분과 두 가지 적분 경로의 분리**

위 식의 적분항을 $\mathcal{I}(V)$라 하고, $u=\left[\frac{1}{2}+\frac{1}{\pi}\arctan\left(\frac{E-\Phi_B(V)}{\eta_0}\right)\right]$, $dv=dE\exp(-E/k_BT)$로 두어 부분 적분($\int udv=uv-\int vdu$)을 수행한다.

$$
\mathcal{I}(V)=\underbrace{\left[-k_BT\exp\left(-\frac{E}{k_BT}\right)\mathcal{T}(E,V)\right]_{E_{min}}^{\infty}}_{\text{Term 1 (Boundary Term)}}+\underbrace{\frac{k_BT\eta_0}{\pi}\int_{E_{min}}^{\infty}\frac{\exp\left(-\frac{E}{k_BT}\right)}{(E-\Phi_B(V))^2+\eta_0^2}dE}_{\text{Term 2 (Kernel Integral)}}
$$
...
$$
\mathcal{I}(V)=k_BT\exp\left(-\frac{E_{min}}{k_BT}\right)\left[\frac{1}{2}+\frac{1}{\pi}\arctan\left(\frac{E_{min}-\Phi_B}{\eta_0}\right)\right]+\frac{k_BT}{\pi}\int_{E_{min}}^{\infty}dE\frac{\eta_0\exp\left(-\frac{E}{k_BT}\right)}{(E-\Phi_B)^2+\eta_0^2}
$$
...
$$
\text{Term 2}=k_BT\int_{E_{min}}^{\infty}dE\exp\left(-\frac{E}{k_BT}\right)\delta(E-\Phi_B)=k_BT\exp\left(-\frac{\Phi_B(V)}{k_BT}\right)
$$

이 항은 이상적인 고전적 다이오드 전류 $J_{ideal}$와 동일하다.

아크탄젠트 테일러 전개 : 밴드 에지 $E_{min}$은 장벽 $\Phi_B$보다 훨씬 아래에 위치하므로, $\Phi_B-E_{min}\gg\eta_0$가 성립한다. $y=(\Phi_B-E_{min})/\eta_0$로 두면 $y\gg 1$이며, $\arctan(-y)\approx-\pi/2+1/y$의 해석적 테일러 전개가 성립한다.
...
$$
\text{Term 1}=\eta_0\left[\frac{k_BT}{\pi(\Phi_B(V)-E_{min})}\exp\left(-\frac{E_{min}}{k_BT}\right)\right]
$$

두 항을 모두 합하면, 전체 전류 밀도 $J(V)$는 두 가지 물리적 성분의 합으로 이루어진다.
...
$$
\mathbf{J}(V)=2e\int\frac{d^3k}{(2\pi)^3}v_z(k_z)\mathcal{T}(E_z,V)\left[f(E,\mu_L)-f(E,\mu_R)\right]
$$

볼츠만 분포 근사($E-\mu\gg k_BT$) 하에서, 지수 함수는 수직 방향과 횡방향으로 분리된다. $\exp(-E/k_BT)=\exp(-E_z/k_BT)\exp(-E_{\perp}/k_BT)$. 이를 수식에 대입하고 적분 변수를 분리한다.

$$
\mathbf{J}(V)=2e\int\frac{dk_z}{2\pi}v_z\mathcal{T}(E_z)e^{-E_z/k_BT}\int\frac{d^2\mathbf{k}_{\perp}}{(2\pi)^2}e^{-E_{\perp}/k_BT}
$$
...
$$
\mathbf{J}(V)=\left(2e\cdot\frac{m^*k_BT}{2\pi(h^2/4\pi^2)}\cdot\frac{1}{h}\right)\int_{E_{min}}^{\infty}dE_z\mathcal{T}(E_z)e^{-E_z/k_BT}
$$
...
% [표준 물성치 및 C-V/SiC 데이터 기준]
V_bi = 1.542;               
N_D_m3 = 2.0e16 * 1e6;     % 에피층 도핑 농도 환산(m^-3)
N_C_m3 = 1.7e19 * 1e6;    
A_star_m2 = 146 * 10000;  % 146 A/cm^2-K^2 -> A/m^2-K^2

%% 2. 데이터 분석 및 전압별 eta_0(V), tau(V) 역산
% SiC_IV.csv 데이터 로드 (1열: 전압 V / 2~9열: 조건별 전류)
data = readmatrix('SiC_IV.csv');
V_raw = data(:, 1);

% [이론적 유효 윈도우 설정] 
% 재결합 소멸 상회 구간(>1.0V) 및 볼츠만 근사 붕괴 이전(<1.465V)
V_min = 1.0; 
V_max = 1.465;

% 조건 설정: (조건명 / 전류데이터 열 번호 / 반경(um))
condition_names = {'Non-rad 450um', 'Non-rad 350um', 'Non-rad 250um', 'Non-rad 150um', ...
                   'Rad 450um', 'Rad 350um', 'Rad 250um', 'Rad 150um'};
col_I_indices   = [2, 3, 4, 5, 6, 7, 8, 9];
radii_um        = [450, 350, 250, 150, 450, 350, 250, 150];

num_conditions = length(condition_names);

figure('Name', 'Interface Quantum Broadening Analysis', 'Color', 'w', 'Position', [100 100 1600 600]);
fprintf('\n========================================================================\n');
fprintf(' [보고서] SBD 계면 투과 완화 시간(tau) 및 양자 산란 뭉개짐(eta_0)\n');
fprintf('------------------------------------------------------------------------\n');
fprintf(' 측정 조건 (Radius)    |   평균 \\eta_0 (neV)   |    평균 \\tau (us)   \n');
fprintf('------------------------------------------------------------------------\n');

for i = 1:num_conditions
    % 면적 계산
    r_um = radii_um(i);
    Area_m2 = pi * (r_um * 1e-6)^2;
    col_I = col_I_indices(i);
    
    % 절대 전류 추출 및 유효 윈도우 인덱싱
    I_raw = abs(data(:, col_I));
    
    idx = (V_raw >= V_min) & (V_raw <= V_max) & ~isnan(V_raw) & ~isnan(I_raw) & (I_raw > 0);
    V_arr = V_raw(idx); 
    J_exp = I_raw(idx) / Area_m2;
    
    if length(V_arr) < 3
        fprintf(' 경고: 유효 전압 구간(%.2fV ~ %.2fV) 내 데이터가 부족합니다. (%s)\n', V_min, V_max, condition_names{i});
        continue; 
    end
    
    % 추출 수식 적용
    Front = pi * q * (V_bi - V_arr);
    Bracket_1 = (J_exp ./ (A_star_m2 * T^2)) .* ((N_C_m3 / N_D_m3) ./ (1 - exp(-(q * V_arr) / kT_J)));
    Bracket_2 = exp(-(q * (V_bi - V_arr)) / kT_J);
    
    eta_0_J = Front .* (Bracket_1 - Bracket_2);
    
    % 단위 스케일 교정
    eta_0_neV = (eta_0_J / q) * 1e9; 
    tau_us = (hbar ./ (2 * eta_0_J)) * 1e6;
    
    % 평균값 계산 및 터미널 출력
    m_eta = mean(eta_0_neV, 'omitnan'); m_tau = mean(tau_us, 'omitnan');
    fprintf(' %-20s | %18.3f      | %18.3f\n', condition_names{i}, m_eta, m_tau);
    
    % 서브플롯: 독립 플로팅
    subplot(2, 4, i);
    yyaxis left; 
    plot(V_arr, eta_0_neV, 'r-', 'LineWidth', 1.5); 
    ylabel('\eta_{0, interface} (neV)');
    
    yyaxis right; 
    semilogy(V_arr, abs(tau_us), 'b-', 'LineWidth', 1.5); 
    ylabel('|\tau_{interface}| (\mus)');
