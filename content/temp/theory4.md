...
### 4. 전역적 비대칭 지표를 이용한 격자 재배치 시나리오

- 사용자가 임의의 격자를 생성(비균일 해도 됨)  
- 물성 맵을 사용하여, $\nabla p$가 튀는 지점을 찾고, 이 부분에 최소한(3개 정도)의 격자를 자동으로 삽입한다.  

```text
[시작: 프리 스캔 및 최악의 x_init 뼈대 생성]  
      │
      ▼
=======================================================================  
[OUTER LOOP: 공간 메트릭 혼합 지수 탐색] - scipy.optimize.minimize_scalar
=======================================================================  
엔진 구동: "0.0 ~ 0.999 사이에서 가장 완벽한 λ 값을 찾아라!"
      │
      ├──▶ 엔진이 테스트 값(예: λ_test = 0.5)을 하나 던짐
      │
      │    ============================================================
      │    [INNER LOOP: 피카르 비선형 동기화] - 최대 50회 (MAX_ITERATIONS)
      │    ============================================================
      │          │
      │          ├──▶ 1. 현재 격자에서 물리량 p(x) 및 구배 S_k 산출
      │          ├──▶ 2. 경계면 0차 외삽 (Sk[0], Sk[-1] 보호)
      │          ├──▶ 3. 위상 매칭 (S_{i+1/2}) 및 강성 행렬 A 조립
      │          ├──▶ 4. 직접 역산 (np.linalg.solve)으로 새 격자(x_next) 도출
      │          │
      │          ├──▶ 5. [조기 종료 검사]: 
      │          │       격자 최대 이동량이 1e-12 미만인가?
      │          │        ├─ (YES) 완벽한 Lock-on! INNER LOOP 즉시 탈출 (Break)
      │          │        └─ (NO) x_curr = x_next 업데이트 후 1번으로 돌아감
      │    ============================================================
      │
      ├──▶ [목적 함수 평가]: INNER LOOP에서 수렴되어 나온 격자(V_k)를 분석
      │    - E_geo (기하학적 찢어짐) + E_phys (물리적 해상도) 점수 계산
      │    - 점수를 최적화 엔진에 보고
      │
      └──▶ 엔진 판단: "점수가 아직 높다. λ 값을 변경하여 다시 돌려보자."
           (최저점을 찾을 때까지 위 과정을 약 10~15회 반복)
...
=======================================================================
      ...
      ▼
=======================================================================
[FINAL EXECUTION: 최종 무균 격자 확정 및 횟수 카운트]
=======================================================================
찾아낸 λ* 를 장전하고, INNER LOOP를 마지막으로 단 1번 풀가동.
조기 종료 시점의 실제 반복 횟수를 기록하고 최종 격자 도출.
      │
      ▼
[종료: 완벽한 격자 데이터 (x_opt, V_k) 반환]
```
...
### 6. 정적 스프링 평형 모델(Static Spring Equilibrium Model)
...
### 7. 브렌트 방법(Brents Method): 미분 없는 지능형 최적화
...
### 9. 격자 가중 행렬 (Grid Weighting Matrix, $\hat{G}$)
...
### 11. 강제 압착 연산자 (Forced Compress Operator, $\hat{C}$)
...
[MACRO LOOP: 물리적 격자 도약 (Physical Grid Adaptation)]
=======================================================================  
엔진 구동: "해의 형태를 추적하여 기생 오차(K)의 싹을 잘라라!"
       │
       ├──▶ 1. [상태 진단] 현재 해 u_m 기반 전역 비대칭 지표 S_k 산출
       │
       ├──▶ 2. [스프링 엔진 가동] 앵커 고정 하에 구역별 최적 λ* 탐색
       │       → 기하학적 K가 최소화된 새 격자 X_new 도출
       │
       ├──▶ 3. [초기해 강제 이식] u_m을 새 격자 X_new에 스플라인 보간
       │       (※ 주의: 보간 오차 발생, Micro 루프에서 세척 예정)
       │
       │    ============================================================
       │    [MICRO LOOP: 대수적 해의 정화 (Algebraic T+K Newton)]
       │    ============================================================
       │    격자 고정: "보간 오차를 씻어내고 10^-16의 무균 해를 찾아라!"
       │          │
       │          ├──▶ a. 정확한 잔차 Res 계산 (보간/K 오차 정보 완벽 탑재)
       │          ├──▶ b. 자코비안 & 메트릭 결합 (A = G * J 조립)
       │          │
       │          ├──▶ c. [★ T+K 대수적 분해 및 K_int 적출]
       │          │        T = (A + A^T) / 2  -> (내부 멸균 골격)
       │          │        K_int = (I-Mb)*K*(I-Mb) -> (RHS로 유배)
       │          │
       │          ├──▶ d. 하이브리드 시스템 풀이 ((T+B) * Δu = - G*Res - K_int*u)
       │          │        (내부 대칭성을 무기로 BiCGSTAB/GMRES 초고속 구동)
       │          │
       │          ├──▶ e. 해 업데이트 (u_n+1 = u_n + Δu)
       │          │
       │          ├──▶ f. [Micro 수렴 판정]: ||Res|| < 10^-16 인가?
       │          │        ├─ (NO) a번으로 돌아가 계속 정화
       │          │        └─ (YES) 완벽 멸균! MICRO LOOP 즉시 탈출 (Break)
       │    ============================================================
       │
       └──▶ 4. [이중 평형 수렴 판정] 내부 격자가 더 이상 움직이지 않는가?
               (||X_new - X_old|| ≒ 0)
                ├─ (NO) X_old = X_new 로 갱신 후 MACRO 1번으로 복귀
                └─ (YES) 격자(물리)와 해(대수) 평형 도달! MACRO LOOP 탈출
...
=======================================================================
       │
       ▼
[종료: 앵커 보존 및 기생 오차 압착 최적 격자 X*, 100% 무균 정해 u* 반환]
```
