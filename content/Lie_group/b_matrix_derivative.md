...
리 군의 매끄러움을 살리려면, 매개변수로 라벨링된 연산자 $\hat{L}(\tau)$를 미분할 수 있어야 한다. 이 장에서는 1-paramete r에서 연산자의 미분 정의(성분별)와 기본 규칙, 그리고 예제를 다룬다. 매개변수에서 **한 점** 만 골랐을 때 얻는 미분값이 무엇을 가리키는지도 짚는다.
...
- 결과는 같은 크기의 행렬이다.
- 행렬 미분은 새로운 개념이 아니라, 성분 하나하나에 보통의 스칼라 미분을 적용한 것에 불과하다.
...
$$\dfrac{d}{d\tau}\cos\tau = -\sin\tau,\quad\dfrac{d}{d\tau}(-\sin\tau) = -\cos\tau$$
$$\dfrac{d}{d\tau}\sin\tau = \cos\tau,\quad\dfrac{d}{d\tau}\cos\tau = -\sin\tau$$
...
(6) 회전 행렬 예에서 $X = \begin{pmatrix} 0 & -1 \\ 1 & 0 \end{pmatrix}$ 이 바로 그 값이다.  
