...
**푸리에 급수는, 기저를 이산화 한 푸리에 역변환이다.** $|f\rangle$이 이산 무한 기저 $|\omega_{n}\rangle_A$로 표현가능하다면,
...
위에서는 푸리에 역변환으로 부터, 푸리에 급수의 본질을 이해하였다. 직관적으로 우리에게 익숙한 테일러 급수 같이 푸리에 급수를 이해할 수 있다. 주기함수일 경우, 주파수의 정수배인 정현파 함수의 합으로 표현할 수 있다.
