...
(고정)위치에서, (아주 작은)물체가 회전하려면, 각 방향에 대한 토크가 0이 아니어야 한다. 그림의 x-y 평면에서, 물체의 x방향의 힘 성분들의 합이 0이 아니거나 또는 y방향의 힘 성분들의 합이 0이 아니어야 한다. 이것은, 공간에 따라 힘의 차이가 존재해야 함을 의미한다. 반시계 방향(기준)으로 회전하려면, 이 회전의 정도를 아래와 같이 표현할 수 있다.
