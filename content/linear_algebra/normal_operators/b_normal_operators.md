...
스펙트럼 정리에 따르면, 이 조건을 만족하는 연산자는 항상 완비적인 정규직교 고유벡터 기저를 가진다는 매우 중요한 특성이 보장된다.

---
### 핵심 특성 및 증명

정규 연산자의 모든 중요한 성질은 다음 특성들로부터 파생되거나 서로 동치 관계에 있다.

#### **특성 1: $\hat{A}$와 $\hat{A}^\dagger$는 고유벡터를 공유한다.**

> **정리**: 만약 $|v\rangle$가 정규 행렬 $\hat{A}$의 고유값 $\lambda$에 대한 고유벡터라면, $|v\rangle$는 동시에 $\hat{A}^\dagger$의 고유값 $\lambda^*$에 대한 고유벡터이기도 하다.
...
#### **특성 2: 서로 다른 고유값의 고유벡터는 직교한다.**

> **증명**: $\hat{A}|v_1\rangle = \lambda_1|v_1\rangle$ 이고 $\hat{A}|v_2\rangle = \lambda_2|v_2\rangle$ 일 때, $\lambda_1 \neq \lambda_2$ 이면 $\langle v_1|v_2\rangle=0$ 이다.
> $\langle v_1|\hat{A}|v_2\rangle = \lambda_2\langle v_1|v_2\rangle$ 이다. 또한, **특성 1**을 이용하면 $\langle v_1|\hat{A}|v_2\rangle = \langle \hat{A}^\dagger v_1|v_2\rangle = \langle \lambda_1^*v_1|v_2\rangle = \lambda_1\langle v_1|v_2\rangle$ 이다.
> 두 결과가 같아야 하므로, $(\lambda_1 - \lambda_2)\langle v_1|v_2\rangle = 0$ 이고, $\lambda_1 \neq \lambda_2$ 이므로 반드시 $\langle v_1|v_2\rangle = 0$ 이어야 한다.
