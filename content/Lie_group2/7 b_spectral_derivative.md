...
§5에서 행렬 미분(성분별), §6에서 함수 미적분과 지수 사상(스펙트럼별)을 봤다. 이 장에서는 두 도구를 결합한다. *스펙트럼 분해된 행렬을 미분하면 어떤 구조가 나오는가?*

매개변수에 따라 고유값과 고유벡터 *둘 다* 변할 수 있는 일반 상황을 분석하고, 고유벡터가 고정되는 깔끔한 경우와 비교한다.
