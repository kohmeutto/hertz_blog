...
## 1. $\hat{M}$-팽창 연산자 ($\hat{M}$-Dilation Operator)

**1) 정의**

$\hat{M}$-정규 연산자($[\hat{D}, \hat{D}^\sharp] = 0$) 중에서, 임의의 상태 벡터에 작용했을 때 그 벡터의 $\hat{M}$-노름을 감소시키지 않는 연산자를 **$\hat{M}$-팽창 연산자**라고 정의한다.

$$
\|\hat{D}\psi\|_M \ge \|\psi\|_M
$$

**2) 물리적 의미**

표준 힐베르트 공간의 계량이 $\hat{M}$으로 확장된 비정규 공간에서, 외부로부터 시스템 내부로 에너지가 유입되거나 신호가 증폭(Gain)되는 능동적인 열린 계를 기술한다. 가역 조건 하에서 $\hat{M}$-수축 연산자와 서로 역연산자($\hat{D} = \hat{T}^{-1}$) 관계를 형성한다.
...
연산자 $\hat{D}$의 고유값 $\lambda$와 영벡터가 아닌 고유벡터 $|\lambda\rangle$에 대하여 다음 고유방정식이 성립한다.
...
$$
\|\hat{D}\lambda\|_M = \|\lambda\lambda\|_M = |\lambda| \|\lambda\|_M
$$

$\hat{M}$-팽창 연산자의 정의 부등식인 $\|\hat{D}\psi\|_M \ge \|\psi\|_M$의 변수 $\psi$ 자리에 고유벡터 $|\lambda\rangle$를 대입한다.

$$
\|\hat{D}\lambda\|_M \ge \|\lambda\|_M
$$

좌변에 앞서 유도한 노름 분해식을 대입하여 정리한다.

$$
|\lambda| \|\lambda\|_M \ge \|\lambda\|_M
$$
...
$\hat{M}$-팽창 연산자 $\hat{D}$는 복소 우반평면에 고유값을 가지는 $\hat{M}$-정규 연산자 $\hat{Z}$의 지수 사상으로 표현되며, 이는 두 개의 가환하는 $\hat{M}$-허미션 연산자의 결합으로 분해된다.
...
$\hat{M}$-정규 연산자 $\hat{D}$는 스펙트럼 정리에 의해 다음과 같이 대각 분해된다.

$$
\hat{D} = \sum_n \lambda_n |\lambda_n\rangle \langle \lambda_n| \hat{M}
$$
...
생성자 고유값 $Z_n$은 복소 우반평면에 존재해야 한다. 지수 함수의 매클로린 급수 정의를 고유기저 전개식에 대입한다.

$$
\hat{D} = \sum_n e^{Z_n} |\lambda_n\rangle \langle \lambda_n| \hat{M} = \sum_n \left( \sum_{k=0}^{\infty} \frac{Z_n^k}{k!} \right) |\lambda_n\rangle \langle \lambda_n| \hat{M}
$$
...
$$
e^{\hat{Z}} = \sum_{k=0}^{\infty} \frac{\hat{Z}^k}{k!} = \sum_n e^{Z_n} |\lambda_n\rangle \langle \lambda_n| \hat{M} = \hat{D}
$$
...
스펙트럼 구속 조건에 의해 $X_n \ge 0$이므로, $\hat{M}$-허미션 연산자 $\hat{H}_R$은 양의 반정부호 연산자이다.
...
| $X_n > 0$ | $Y_n \in \mathbb{R}$ | $\vert\lambda_n\vert > 1$ | 증폭 진동 (일반 개방계 증폭 상태) |