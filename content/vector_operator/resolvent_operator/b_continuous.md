...
이 관계에는 번호가 둘 붙어 있다. 하나는 고윳값을 가리키는 번호 $m$ 이고, 다른 하나는 사슬에서 몇 번째 칸인지 가리키는 번호 $k$ 다. 연속화는 이 둘 가운데 고윳값 번호 $m$ 하나만 연속 라벨 $\nu$ 로 바꾼다. 사슬 칸 번호 $k$ 는 그대로 둔다. 사슬 칸이 왜 정수로 남는지는 §2에서 두 축을 가르며 따로 본다. 여기서는 고윳값 쪽만 연속으로 옮긴다.

여기서 라벨과 고윳값을 구별해 둔다. 이산에서는 번호 $m$ 이 고윳값 $\lambda_m$ 을 골랐다. 연속에서는 라벨 $\nu$ 가 스펙트럼을 훑고, 그 라벨에서의 고윳값이 $\nu$ 의 함수 $\lambda(\nu)$ 가 된다. 곧 $m$ 번째 고윳값 $\lambda_m$ 이 연속에서 $\lambda(\nu)$ 로 바뀐다. 위치 연산자처럼 라벨이 곧 고윳값인 경우($\lambda(\nu) = \nu$)도 있고, 밴드 구조처럼 라벨(이를테면 파수)과 고윳값(이를테면 에너지)이 다른 경우도 있으므로, 고윳값을 라벨과 같은 글자로 적지 않고 $\lambda(\nu)$ 로 적는다.
...
사슬 켓 하나에는 두 정보가 들어 있다. 사슬의 몇 번째 칸 $k$ 인가, 그리고 어느 라벨 $\nu$ 에 속하는가다. 텐서곱 챕터에서 위치와 스핀처럼 성격이 다른 두 자유도를 한 켓에 담았던 것과 같은 상황이다. 두 정보는 서로 독립이다. 같은 라벨 $\nu$ 위에 사슬 칸이 여러 개 쌓일 수 있고, 같은 칸 번호 $k$ 가 여러 라벨에 걸쳐 나타난다. 이렇게 독립인 두 자유도를 한 켓에 묶는 연산이 텐서곱이다.

여기서 두 자유도를 어느 자리에 둘지 정한다. 사슬 칸을 왼쪽에, 라벨을 오른쪽에 둔다. 사슬 칸이 왼쪽에 오는 까닭은, 사슬 칸이 블록의 자리(위 칸인지 아래 칸인지)를 정하고 그 자리에 라벨 켓 $|\nu\rangle$ 가 들어가기 때문이다. 이 순서가 아래 4번에서 세울 블록 연산자, 그리고 §6의 구체 예제와 맞아떨어진다.

$$
|\lambda_k(\nu)\rangle = |e_k\rangle \otimes |\nu\rangle
$$

왼쪽 인자 $|e_k\rangle$ 는 사슬 축을 맡는다. 사슬 길이가 $K$ 이면 $|e_k\rangle$ 는 $\mathbb{C}^K$ 의 표준기저, 곧 $k$ 번째 자리만 1이고 나머지가 0인 열벡터로, 사슬의 어느 칸인지를 가리킨다. 오른쪽 인자 $|\nu\rangle$ 는 고윳값 축을 맡으며, 라벨 $\nu$ 로 매겨진 고유켓이고 그 고윳값은 $\lambda(\nu)$ 다. 위치 고유켓처럼 연속 라벨로 매겨진다.
...
이제부터 고윳값 축 연산자 $\hat{X}_\nu$ 의 구체 보기로 위치 연산자 $\hat{X}$ 를 쓴다. 위치 연산자는 위치값이 곧 고윳값인 특수한 보기, 곧 라벨과 고윳값이 같은 $\lambda(\nu) = \nu$ 인 경우다. 위치 고유켓 $|\nu\rangle$ 에 $\hat{X}|\nu\rangle = \nu|\nu\rangle$ 로 작용하므로, $\hat{X}_\nu$ 의 정의 $\hat{X}_\nu|\nu\rangle = \lambda(\nu)|\nu\rangle$ 에 $\lambda(\nu) = \nu$ 를 넣은 것과 같다. 이 보기로 블록 연산자를 적는다.

$$
\hat{A} = \begin{pmatrix} \hat{X} & \hat{I} \\ 0 & \hat{X} \end{pmatrix}
$$

이 연산자가 §4 이후 연속 분해의 기본 보기이고, §6에서 구체 예제로 다시 확인한다. 일반 라벨에서는 대각에 고윳값 $\lambda(\nu)$ 가 깔리지만, 위치 연산자 보기에서는 $\lambda(\nu) = \nu$ 라 대각의 고윳값이 $\nu$ 가 된다.
...
다음 연산자를 본다. 네 자리가 모두 비영이라, 겉으로는 블록 꼴도 사슬 구조도 보이지 않는다. 이 예제는 고윳값 축 연산자로 위치 연산자 $\hat{X}$ 를 쓰는 구체 보기이므로 라벨과 고윳값이 같은 $\lambda(\nu) = \nu$ 인 경우이고, 따라서 아래에서 고윳값은 $\nu$ 로 나온다.
...
고윳값을 행렬식으로 찾는다. 고윳값을 구하는 변수는 고윳값 함수 $\lambda(\nu)$ 와 글자가 겹치지 않게 $\mu$ 로 둔다. 행렬식 정의 $\det\begin{pmatrix} a & b \\ c & d \end{pmatrix} = ad - bc$ 에 $a = \nu+1-\mu$, $b = 1$, $c = -1$, $d = \nu-1-\mu$ 를 대입한다.
...
이중근 $\mu = \nu$ 다. 곧 이 보기에서 $\hat{A}'$ 의 고윳값이 위치값 $\nu$ 와 같으니, $\lambda(\nu) = \nu$ 가 그대로 확인된다. 진성 고유벡터가 몇 개인지 보려고 $M(\nu) - \nu\hat{I}_2$ 를 적는다. 대각 성분은 $(\nu+1) - \nu = 1$ 과 $(\nu-1) - \nu = -1$ 이다.
...
이산 사슬 관계, 곧 자기 고윳값을 돌려주고 한 칸 아래를 내놓는 관계가 연속 라벨 $\nu$ 위에서 그대로 성립한다. 여기서 고윳값이 $\nu$ 인 것은 일반 지배 방정식 $\hat{A}|\lambda_k(\nu)\rangle = \lambda(\nu)|\lambda_k(\nu)\rangle + |\lambda_{k-1}(\nu)\rangle$ 에 위치 연산자 보기의 $\lambda(\nu) = \nu$ 를 넣은 것이다.
