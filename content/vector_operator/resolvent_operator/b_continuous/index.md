...
### 2. 극한 흡수 원리와 소호츠키-플레멜 정리의 결합: 스톤의 공식 도출

이산 공간에서 코시 닫힌 경로 적분 $\oint_{\Gamma_m}$ 을 통해 단일 사영 연산자 $\hat{P}_m$ 을 유도했던 대수적 과정은, 연속 공간에서 분지절단면을 가로지르는 레졸번트의 도약(Jump) 연산으로 치환된다.
...
특이점 구간 $[\nu_1, \nu_2]$ 을 둘러싸는 직사각형 경로 $\Gamma$ 를 실수축에 무한히 밀착시키는 극한 흡수 원리(Limiting absorption principle)를 적용한다.

![분지절단과 도약 연산](image1.svg)

수직 구간의 적분은 소멸하고, 실수축과 평행하게 주행하는 상단 경로($z = \nu + i\epsilon$, 역방향 주행)와 하단 경로($z = \nu - i\epsilon$, 순방향 주행)의 적분만이 남아 다음의 극한 차분식을 형성한다.

$$
\Delta\hat{P}_{[\nu_1, \nu_2]} = \lim_{\epsilon \to 0^+} \frac{1}{2\pi i} \int_{\nu_1}^{\nu_2} \left[ R(\nu - i\epsilon, \hat{A}) - R(\nu + i\epsilon, \hat{A}) \right] d\nu
$$

적분 구간을 국소적인 미소 척도 $d\nu$ 로 한정하면, 사영 밀도 $d\hat{P}(\nu)$ 를 추출하기 위한 대수적 원형이 도출된다. 

$$
d\hat{P}(\nu) = \lim_{\epsilon \to 0^+} \frac{1}{2\pi i} d\nu \left[ R(\nu - i\epsilon, \hat{A}) - R(\nu + i\epsilon, \hat{A}) \right] 
$$

괄호 내부의 레졸번트 차분을 해석학적으로 연산하기 위해 복소 분수 함수의 극한을 규명하는 소호츠키-플레멜 정리(Sokhotski-Plemelj theorem)를 도입한다. 이 정리는 실수 변수 $x$ 에 대하여 특이점을 우회하는 적분 경로의 기하학적 구조를 다음과 같은 분포 이론(Distribution theory) 항등식으로 정의한다.

$$
\lim_{\epsilon \to 0^+} \frac{1}{x \pm i\epsilon} = \mathcal{P} \left( \frac{1}{x} \right) \mp i\pi \delta(x)
$$

독립 변수 $x$ 를 연속 스펙트럼의 관측점 $\nu$ 와 시스템 연산자 $\hat{A}$ 의 대수적 차이인 $(\nu - \hat{A})$ 로 치환하여 레졸번트 연산자에 정리를 전개한다. 좌변의 레졸번트 극한은 발산하지 않는 코시 주치(Cauchy Principal Value, $\mathcal{P}$) 성분과 특이점에서의 붕괴 성분인 디랙 델타 함수(Dirac delta function) 성분으로 유도적으로 분할된다.

$$
R(\nu \pm i\epsilon, \hat{A}) = \mathcal{P} \left( \frac{1}{\nu - \hat{A}} \right) \mp i\pi \delta(\nu - \hat{A})
$$

이 항등식을 도약 연산식에 대입하여 상단 극한과 하단 극한의 대수적 차분을 산출한다.
...
극한 흡수 원리와 소호츠키-플레멜 정리의 해석학적 결합을 통해, 임의의 자기수반 연산자 스펙트럼 측도를 레졸번트의 도약으로부터  역추출해 내는 이 최종 대수적 결과물을 스톤의 공식(Stone's formula)이라 명명한다.

proof) 소호츠키-플레멜 정리의 증명

![소호츠키-플레멜 정리의 기하학적 적분 경로](image2.svg)

*(1) 복소 분수의 대수적 분리 (실수화)*

극한 대상이 되는 복소 분수 함수 $f_{\epsilon}(x) = \frac{1}{x \pm i\epsilon}$ 에 켤레 복소수 $x \mp i\epsilon$ 를 분모와 분자에 곱하여 실수부와 허수부로  분리한다.
...
실수부와 허수부의 극한 결과를 본래의 분리 식에 대입하면, 어떠한 해석학적 비약 없이 소호츠키-플레멜 정리가 유도적으로 완성된다.
...
우변의 첫 번째 항은 $\nu d\hat{P}(\nu)$ 로 정의되며, 두 번째 항은 이산 공간의 2차 극점 계수 유도($\hat{C}_{-2} = \oint (z-\lambda_m)R(z) dz$) 논리를 연속 공간으로 확장한 멱영 밀도 $d\hat{N}(\nu)$ 와 일치한다. 
...
연속 공간 상호 기저의 직교 규격화 조건은 미소 구간 밀도 간의 곱을 통해 델타 함수 형태의 사영 측도로 대수적으로 보장된다. 제1 레졸번트 항등식을 극한 흡수 원리에 대입하여 밀도의 직교성을 증명한다.
...
2장의 소호츠키-플레멜 정리 전개 과정에서 확인하였듯, 특이점을 포함하는 적분 구간은 델타 함수(사영 밀도) 성분과 코시 주치 성분으로 분리된다. 스톤의 공식 도출 과정에서 발산하는 델타 함수를 축출하고 상쇄시켰던 바로 그 코시 주치(Cauchy Principal Value, $\mathcal{P}$) 성분이 연속 공간의 환원 레졸번트 $\hat{S}(\nu)$ 를 대수적으로 정의한다.
...
이산 공간의 예제(행렬)와 대비하여, 무한 차원 힐베르트 공간 $L^2(\mathbb{R})$ 상에서 작용하는 자기수반(Self-adjoint) 연속 연산자인 위치 연산자 $\hat{X}$ 를 통해 극한 흡수 원리와 스톤의 공식이 어떻게 연속 사영 밀도 $d\hat{P}(\nu)$ 를 대수적으로 유도해 내는지 교차 검증한다.
...
위치 연산자 $\hat{X}$ 는 임의의 실수 $x \in \mathbb{R}$ 전체 구간에 대하여 연속적인 고윳값을 가진다. 무한 차원 공간이므로 $\det(\hat{X} - \lambda\hat{I}) = 0$ 이라는 행렬식 기반의 대수적 특성 방정식은 성립하지 않으며, 고윳값들은 실수축 전체를 덮는 분지절단(Branch cut) $\Omega = (-\infty, \infty)$ 을 형성한다.
...
도출된 차분 결과에 $\frac{1}{2\pi i}$ 를 곱하고 $\epsilon \to 0^+$ 극한을 취한다.
...
소호츠키-플레멜 정리의 전개 결과에 따라, 우변의 형태는 분포 이론 상의 디랙 델타 함수 $\delta(x)$ 로  수렴한다.
...
유도된 사영 밀도를 기반으로 대상 연산자를 전체 스펙트럼 구간 $\Omega$ 에 대하여 적분한다.
...
결과적으로 스톤의 공식과 소호츠키-플레멜 정리의 해석학적 결합이, 어떠한 논리적 비약 없이 연속 기저를 투영하는 사영 밀도를 정확하게 생성해 냄이 확증된다.
